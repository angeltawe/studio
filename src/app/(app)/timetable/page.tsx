    ...
    <div className="container mx-auto p-4 md:p-8 pb-24 md:pb-8">
      <div className="flex flex-col lg:flex-row gap-6 md:gap-8">
        ...
        <div className="w-full lg:w-80 space-y-4 md:space-y-6">
          <Card className="rounded-xl md:rounded-3xl shadow-sm border-none bg-white p-2 md:p-4">
            <Calendar
              mode="single"
              selected={date}
              onSelect={setDate}
          ...
          <Card className="rounded-xl md:rounded-3xl shadow-sm border-none bg-primary/5 p-4 md:p-6 hidden lg:block">
        ...
        <div className="flex-1 space-y-4 md:space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0">
              <h1 className="text-xl md:text-3xl font-headline font-bold truncate">My Schedule</h1>
          ...
          <div className="space-y-3 md:space-y-4">
            {mockSchedule.map((session) => (
              <Card key={session.id} className="rounded-xl md:rounded-3xl shadow-sm border-border group overflow-hidden hover:shadow-md transition-all">
                <div className="flex flex-col sm:flex-row min-h-full">
                  <div className={cn("h-1.5 sm:h-auto sm:w-2 md:w-3", session.color)} />
                  <CardContent className="flex-1 p-4 md:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-3 md:gap-4 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <h3 className="text-sm md:text-xl font-bold font-headline truncate">{session.subject}</h3>
                          <Badge variant="secondary" className="text-[8px] md:text-[10px] uppercase font-bold">{session.type}</Badge>
                        </div>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] md:text-sm text-muted-foreground">
                        ...
                        {session.notes && (
                          <p className="mt-2 text-[9px] md:text-sm text-muted-foreground italic border-l-2 border-primary/20 pl-2 md:pl-3 line-clamp-2">
                            "{session.notes}"
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-end gap-2 shrink-0">
                      ...
                      <Button className="h-8 md:h-10 text-[10px] md:text-sm rounded-lg md:rounded-xl px-4 md:px-6 bg-primary/10 text-primary hover:bg-primary hover:text-white border-none shadow-none flex-1 sm:flex-none font-bold">
                        Mark as Done
                      </Button>
                    </div>
                  </CardContent>
